...
class TestEgressEmit:
    def test_egress_emitted_with_tables(self):
        ...
        assert "control TestEgress(" in p4_src
        assert "table mac_table {" in p4_src
        assert "action rewrite(bit<48> src)" in p4_src
        assert "mac_table.apply();" in p4_src
        # Main instantiation should use TestEgress, not MyEgress
        assert "TestEgress()" in p4_src
        assert "MyEgress()" not in p4_src
    ...
    def test_no_egress_emits_empty_placeholder(self):
        ...
        program = compile(main)
        p4_src = emit(program)
        assert "control MyEgress(" in p4_src
        assert "MyEgress()" in p4_src
